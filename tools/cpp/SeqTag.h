...
#define HIC_TOTAL_READS_NORMALIZATION 1e8
...
#define HIC_MASK_NORM_BALANCED_SEQDEPTH 0x80000

#define HIC_DLR_DEFAULT_DISTANCE 3000000
#define HIC_DLR_DEFAULT_MINDISTANCE 1000
...
#define PEAK_SORT_BY_CHR_START_COORDINATE 0 
#define PEAK_SORT_BY_CHR_END_POS_COORDINATE 1

#define PARSE_SAM_RMSOFT_EXTEND 0
#define PARSE_SAM_RMSOFT_TRIMSOFT 1
#define PARSE_SAM_RMSOFT_DELETESOFT 2
...
public:
	...
	//internal functions
	void readAlignment(char* files, int format, int mode, int PEflag,int rmsoftFlag);
	void readPEAlignment(char* files, int format, int mode,int rmsoftFlag);
	...
	int getRightCoordFromCIGAR(char* str, int dir, char* cigarCodes, int* cigarLens, int &numCodes, int &initLen,int rmsoftFlag);
	...
	void normalizeTagCountsLength(char* lenCtrlFile, double minFold,double maxFold,
																	double maxPerror);
	...
	void makeHiCBgModelSimple(HiCBgModel* model,HiCparams* params);
	...
	void getPETagTotals(PeakLibrary* peaks, int balanceEstFlag);
	void getPETagTotalsThread(int cpu, PeakLibrary* peaks);
	void balancePETagTotals(PeakLibrary* peaks);
...
//---
	...
	void print2dBED(FILE* fp);
	int read(char* file); // returns resolution estimate
	int read2dBED(char* file); // returns resolution estimate
...
public: 
	...
	double maxCoverage;
	double minCoverage;
	double minExpectFraction;
	double minExpectCoverage;
	double totalModelReads;
	double totalReads;
	double noDistNormTotal;
	double modelError;
	int goodRegions;
	int badRegions;
	int customFlag;
	HiCBgModelChr *currentRegions;
	PeakLibrary* refPeaks;
	pthread_mutex_t mutex;
	...
	void initializeCurrentRegions(int resolution);
	void printCurrentRegions(FILE* fp);
...
public:
	...
	void normalizeTagCountsLength(double **);
...
public:
	...
	double minExpectRatio;
	double totalNormReads;
...
int cmpPeaksEndPos(const void*, const void*);
...
class PEFreqDistribution {
public:
	TagLibrary* tagDir;
	char* filename;

	double* fileFreq;
	double fileFreqInterChr;
	int fileFreqLength;
	int fileFreqRes;
	...
	PEFreqDistribution(TagLibrary* tagdir);
	~PEFreqDistribution();
	void readPEFreqFile(char* filename);
	void setRes(int res);
	void printFreq(FILE* fp);
